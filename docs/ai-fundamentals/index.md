...
```mermaid
graph LR
    A[机器学习] --> B[监督学习 Supervised 有标注数据]
    A --> C[无监督学习 Unsupervised 无标注数据]
    A --> D[强化学习 Reinforcement 奖惩机制]
```
...
```mermaid
graph LR
    A[输入文字] --> B[Tokenizer 分词]
    B --> C[Transformer 理解上下文]
...
```mermaid
graph TD
    A[预训练 Pre-training 海量文本学语言规律] --> B[指令微调 Instruction Tuning 学会按指令回答]
    B --> C[RLHF 人类反馈强化学习 对齐人类偏好]
    C --> D[ChatGPT / Claude]
```
...
```mermaid
graph TB
    A[LLM API OpenAI / Claude / Gemini] --> B[应用层]
    B --> C[Prompt Engineering 提示词工程]
    B --> D[RAG 检索增强生成]
    B --> E[Function Calling 工具调用]
    B --> F[Fine-tuning 模型微调]
```
